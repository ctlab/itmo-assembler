 ...
 */
public class Long2ShortHashMap extends LongHashSet implements Long2ShortHashMapInterface {
    ...
    // constructors
    public Long2ShortHashMap() {
        this(20, DEFAULT_MAX_LOAD_FACTOR);  // 1 M elements
    }
    public Long2ShortHashMap(int capacity) {
        this(
                NumUtils.getPowerOf2(capacity),
                DEFAULT_MAX_LOAD_FACTOR
        );
    }
    public Long2ShortHashMap(int logCapacity, float maxLoadFactor) {
        if (logCapacity > 30) {
    ...
    public short put(long key, short value) {
        if (key == FREE) {
            ...
            try {
                ...
                short prev = curData.valueForFreeKey;
    ...
    public void reset() {
        ...
        try {
            ...
            Arrays.fill(curData.values, (short)0);
    ...
    public void write(DataOutput out) throws IOException {
        ...
        for (int i = 0; i < curData.capacity; i++) {
            ...
            out.writeShort(curData.values[i]);
        }
        out.writeBoolean(curData.containsFreeKey);
        out.writeShort(curData.valueForFreeKey);
    }
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        for (int i = 0; i < capacity; i++) {
            ...
            newData.values[i] = in.readShort();
        }
        newData.containsFreeKey = in.readBoolean();
        newData.valueForFreeKey = in.readShort();
        newData.size = size;
    ...
    protected class MyIterator implements Iterator<MutableLongShortEntry> {
        private final MapData curData;
        private int index = 0;
        private final MutableLongShortEntry entry = new MutableLongShortEntry();
