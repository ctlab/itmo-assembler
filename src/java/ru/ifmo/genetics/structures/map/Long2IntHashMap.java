...
public class Long2IntHashMap extends LongHashSet implements Long2IntHashMapInterface {
    ...
    public Long2IntHashMap(int capacity) {
        this(
                NumUtils.getPowerOf2(capacity),
                DEFAULT_MAX_LOAD_FACTOR
        );
    }
    public Long2IntHashMap(int logCapacity, float maxLoadFactor) {
        if (logCapacity > 30) {
    ...
    public int put(long key, int value) {
        if (key == FREE) {
            ...
            try {
                ...
                int prev = curData.valueForFreeKey;
    ...
    public void write(DataOutput out) throws IOException {
        ...
        for (int i = 0; i < curData.capacity; i++) {
            ...
            out.writeInt(curData.values[i]);
        }
        out.writeBoolean(curData.containsFreeKey);
        out.writeInt(curData.valueForFreeKey);
    }
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        for (int i = 0; i < capacity; i++) {
            ...
            newData.values[i] = in.readInt();
        }
        newData.containsFreeKey = in.readBoolean();
        newData.valueForFreeKey = in.readInt();
        newData.size = size;
    ...
    protected class MyIterator implements Iterator<MutableLongIntEntry> {
