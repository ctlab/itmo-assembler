 ...
 * <br></br>
 */
public class BigLong2LongHashMap implements Long2LongHashMapInterface {
    private static final Logger logger = Logger.getLogger("BigLong2LongHashMap");

    public Long2LongHashMap[] maps;
    protected int mask;


    public BigLong2LongHashMap(long capacity) {
        this(
    ...
    public BigLong2LongHashMap(int logSmallMapNumber, int logSmallCapacity) {
        this(logSmallMapNumber, logSmallCapacity, false);
    }

    public BigLong2LongHashMap(int logSmallMapNumber, int logSmallCapacity, boolean debugInfo) {
        ...
        maps = new Long2LongHashMap[smallMapNumber];
        for (int i = 0; i < smallMapNumber; i++) {
            maps[i] = new Long2LongHashMap(logSmallCapacity, LongHashSet.DEFAULT_MAX_LOAD_FACTOR);
        }
        if (debugInfo) {
            Tool.debug(logger, "Created " + NumUtils.groupDigits(smallMapNumber) + " small Long2LongHashMaps");
        }
    }
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        maps = new Long2LongHashMap[len];
        mask = maps.length - 1;

        for (int i = 0; i < len; i++) {
            maps[i] = new Long2LongHashMap(2);
    ...
    class MyIterator implements Iterator<MutableLongLongEntry> {
        private int index;
        private Iterator<MutableLongLongEntry> it = null;
